...
class UsersUltraMessagingAdmin {
	...
	function admin_init() 
	{
		...
		$this->tabs = array(
			'manage' => __('Messages','xoousers'),
			'send' => __('Send Message','xoousers')
	...
	function add_menu()
	{
		add_submenu_page( 'userultra', __('Messaging Ultra','xoousers'), __('Messaging Ultra','xoousers'), 'manage_options', 'userultra-messaging', array(&$this, 'admin_page') );
	...
	function uultra_send_to_users()
	{
		...
		echo '<div class="updated"><p><strong>'.__('Message Sent!.','xoousers').'</strong></p></div>';
	...
	function admin_page() {
			...
			
		{
			if($_POST['uu_subject']=="" || $_POST['uu_message']=="")
			{
				...
				echo '<div class="error"><p><strong>'.__('ERROR: All fields are required.','xoousers').'</strong></p></div>';
