...
class XooShortCode {
	...
	*/
	...
	{	
		...
		add_shortcode( 'usersultra_user_custom_meta_info', array(&$this,'usersultra_user_custom_meta_info_function') );			
	...
	*/
	...
	public function  usersultra_login_function ($atts)
	{
		...
		if (isset($_GET['resskey'])&& $_GET['resskey']!="") 
		{
			...
			if (!is_user_logged_in()) 
			{
				...
				{
					$xoouserultra->login->auto_redirection_on_login($atts);
	...
	public function  usersultra_user_custom_meta_info_function ($atts)
	{
		global $xoouserultra;
		return $xoouserultra->userpanel->show_user_custom_metainfo($atts );
	 ...
	 */
	 ...
	 function respo_get_period($period, $type, $length, $amount)
	 {
		 $html = '';
		 
		 //type //onetime or recurring
		 //period //M months , W weeks, D days, Y years
		 //length, numbers	
		 ...
		 if($length>1 ){$period = $period.$plural_variable ; }
		 
		 if($length>0 && $type=='recurring')
		 {
			$html = __('every ','xoousers') .$length.$period;
		
		 }
		 
		 if($type=='onetime')
		 {
			$html = __('one-time ','xoousers') ;
	 ...
	 function respo_pricing_shortcode( $atts, $content = null  ) 
	 {
		...
		$amount = number_format($amount,$pricing_decimals,".",",") ;
		...
		$period = $package->package_time_period; // M months , W weeks
		$package_type = $package->package_type; //onetime or recurring
		$billing_length = $package->package_number_of_times;
		
		
		
				
		$per = $this->respo_get_period($period, $package_type, $billing_length, $amount);
		
		//custom text for free packages		
		$free_text =  $xoouserultra->get_option('membership_display_zero');
		$amount_text = $currency_symbol. $amount. $next_to_price;
