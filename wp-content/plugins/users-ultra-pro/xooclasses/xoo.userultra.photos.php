...
class XooUserPhoto {
				...
				{								
					...
					case "youtube":
					 ...
					 if(filter_var($video->video_unique_vid, FILTER_VALIDATE_URL, FILTER_VALIDATE_URL)) //yes URL
					 {
						...
						$html .= '<iframe width="99%" src="'.$url.'" frameborder="0" webkitAllowFullScreen mozallowfullscreen allowFullScreen></iframe>';
  ...
  $html .="</select>
	...
	public function edit_gallery ()
	{
		...
		if($gal_id!="")
		{
			...
			$res = $wpdb->get_results( 'SELECT *  FROM ' . $wpdb->prefix . 'usersultra_galleries WHERE `gallery_id` = ' . (int)$gal_id . ' AND `gallery_user_id` = ' . $user_id . ' ' );
  ...
  <option value='2' ".$friends."> ".__( 'Only Friends', 'xoousers' )."</option>
	...
	public function edit_video ()
	{
		...
		if($video_id!="")
		{
			...
			$res = $wpdb->get_results( 'SELECT *  FROM ' . $wpdb->prefix . 'usersultra_videos WHERE `video_id` = ' . (int)$video_id . ' AND `video_user_id` = ' . $user_id . ' ' );
  ...
  <option value='vimeo' ".$vimeo.">Vimeo</option>
   <option value='embed' ".$embed.">Embed</option>
	...
	public function get_photos_of_gal_public ($gal_id, $display_photo_rating, $display_photo_description, $display_photo_title, $gallery_type)
	{
		global $wpdb, $xoouserultra;
		...
		if (isset($gal_id)) 
		{
			...
			if ( empty( $photos ) )
			{
				...
				foreach ( $photos as $photo )
				{
					...
					if($display_photo_title!='no')
					{
						
						$html.= "<p class='uu-front-prof-photo-title' >".$photo->photo_name."</p>";
					...
					if($gallery_type=="lightbox")
					{
						...
						$html .="<a href='".$large."' class='' data-lightbox='example-1' data-title='".$photo->photo_desc."'><img src='".$thumb."'  title='".$photo->photo_name."' /> </a>";
					
				    
					}else{
						
												
					$html .= "<a href='".$xoouserultra->userpanel->public_profile_get_photo_link($photo->photo_id, $user_id)."' class='' ><img src='".$thumb."' title='".$photo->photo_name."' /> </a>";
	...
	public function get_single_photo($photo_id, $user_id, $display_photo_rating, $display_photo_description)
	{
		...
		if (!$error) 
		{
			...
			$html.= "<a href='#' class='' title='".$current_photo->photo_name."'><img src='".$thumb."' class='rounded'/> </a>";
...
?>
		...
		<div id="uploadContainer" style="margin-top: 10px;">
			...
			<div id="uploaderSection" style="position: relative;">
				<div id="plupload-upload-ui" class="hide-if-no-js">
					...
					<div id="drag-drop-area-gallery">
						<div class="drag-drop-inside">
							<p class="drag-drop-info"><?php	_e('Drop files here', 'xoousers') ; ?></p>
							<p><?php _ex('or', 'Uploader: Drop files here - or - Select Files'); ?></p>
							                            
                                                      
                            <p><button id="plupload-browse-button-photogallery">  <?php _e('Select Files', 'xoousers'); ?></button></p>
                            
														
						</div>
                        
                        <div id="progressbar-uultra-gallery"></div>                 
                         <div id="symposium_filelist_gallery" class="cb-uultra"></div>
					</div>
				</div>
		...
		<?php
			...
			$plupload_init = array(
				...
				'browse_button'       => 'plupload-browse-button-photogallery',
				'container'           => 'plupload-upload-ui',
				'drop_element'        => 'drag-drop-area-gallery',
				...
				jQuery(document).ready(function($){
					...
					uploader.bind('Init', function(up){
						...
						if(up.features.dragdrop){
							...
							$('#drag-drop-area-gallery').unbind('.wp-uploader');
						}
					...
					uploader.bind('FilesAdded', function(up, files) {
						jQuery.each(files, function(i, file) {
							jQuery('#symposium_filelist_gallery').append('<div class="addedFile" id="' + file.id + '">' + file.name + '</div>');
						});
						up.refresh(); 
						uploader.start();
					});
					...
					uploader.bind('UploadProgress', function(up, file) {
						...
						jQuery('#progressbar-uultra-gallery').fadeIn().progressbar({
							value: progressBarValue
						});
						
						jQuery('#progressbar-uultra-gallery .ui-progressbar-value').html('<span class="progressTooltip">' + up.total.percent + '%</span>');
					});
