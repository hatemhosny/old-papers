...
<?php if($_GET["action_order"]=='edit' && $_GET["order_id"] !='' ){
 ...
 <div class="user-ultra-sect ">
        ...
        <h3><?php _e('Edit Order:','xoousers'); ?></h3>  
          ...
          <form action="" method="get">
             <input type="hidden" name="page" value="userultra" />
              ...
              <p><?php _e('Transaction ID','xoousers'); ?>: </p> 
              <p> <input type="text" name="transaction_id" id="transaction_id" value="<?php echo $order->order_txt_id ?>" /></p> 
              
              <p><?php _e('Status','xoousers'); ?>: </p>
            <p><select name="p_order_status">
                <option value="pending" <?php if($order->order_status=='pending'){echo 'selected="selected"';}?> ><?php _e('pending','xoousers'); ?></option>
                <option value="confirmed" <?php if($order->order_status=='confirmed'){echo 'selected="selected"';}?>><?php _e('confirmed','xoousers'); ?></option>
            </select> </p>
                
              <p><?php _e('Payment Method','xoousers'); ?>: </p> 
              <p>
           ...
           <table width="100%" class="wp-list-table widefat fixed posts table-generic">
            <thead>
                <tr>
                    ...
                    <th width="11%"><?php _e('Date', 'xoousers'); ?></th>
                     <th width="11%"><?php _e('Expiration', 'xoousers'); ?></th>
                    ...
                    <th width="9%"><?php _e('Action', 'xoousers'); ?></th>
                </tr>
            </thead>
			...
			?>
                ...
                <tr>
                   ...
                   <td> <a class="uultra-btn-deletemessage " href="?page=userultra&tab=orders&action_order=edit&order_id=<?php echo  $order->order_key?>" id="" title="<?php _e('Edit Order','xoousers'); ?>" order-id="<?php echo $order->order_id?>" ><span><i class="fa fa-edit fa-lg"></i></span></a></td>
                </tr>
