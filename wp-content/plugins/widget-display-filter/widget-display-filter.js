jQuery(document).ready(function ($) {
    ...
    WidgetFilterCssclass = function(nonce, hashtag){
        ...
        csvcc = csvcc.slice(0,-1);
        $('#filter-cssclass').val(csvcc);

        $( "#cssclass-dialog" ).dialog({
            ...
            buttons     : {  
                "O K": function() {
                    var csvcc = $('#filter-cssclass').val();
                    $.ajax({ 
                        type: 'POST', 
                        url: ajaxurl,
                        data: {
                            action: "Widget_filter_cssclass", 
                            hashtag: hashtag, 
                            _ajax_nonce: nonce, 
                            cssclass: csvcc,
                        }, 
                        ...
                        success: function(response, dataType) {
                            if(response.success){
                                //current css class name data update
                                $('#widget-filter-cssclass-' + hashtag).html(response.data);
                                csvcc = $("#widget-filter-cssclass-" + hashtag + " > span").text();
                                var arcc = new Array();
                                arcc = csvcc.split(",");
                                widget_filter[hashtag]['cssclass'] = arcc;
                            }
                            else {
                                alert('CSS class Error : ' + response.data);
                            }
                        },
